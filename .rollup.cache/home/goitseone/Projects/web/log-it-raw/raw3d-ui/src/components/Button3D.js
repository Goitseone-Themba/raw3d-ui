 ...
 * ```
 */
export const Button3D = ({ children, className, 'aria-label': ariaLabel, variant = 'drawn' }) => {
    const { soundOn } = useContext(SoundContext);
    const playSound = () => {
        if (soundOn)
            new Audio(clickSound).play();
    };
    return (_jsx(_Fragment, { children: variant === 'keycap' ? (_jsxs(KeycapButton, { className: className, onClick: playSound, "aria-label": ariaLabel, children: [_jsx("div", { className: "top", children: children }), _jsx("div", { className: "bottom" }), _jsx("div", { className: "base" })] })) : (_jsx(DrawnButton, { className: className, onClick: playSound, "aria-label": ariaLabel, children: children })) }));
};
