...
interface Button3DProps {
  ...
  className?: string;
  'aria-label'?: string;
  variant?: 'original' | 'modern'; // Add variant prop
}

export const Button3D = ({ children, className, 'aria-label': ariaLabel, variant = 'original' }: Button3DProps) => {
  ...
  if (variant === 'modern') {
    // Modern variant with Framer Motion (new design)
    return (
      <button
      ...
      >
        <div className="absolute w-full h-full bg-gradient-to-br from-[#f5f5e8] to-[#e0e0d0] font-roboto text-2xl font-medium text-[#2a2a2a] flex items-center justify-center rounded-lg border-2 border-[#2a2a2a] shadow-[inset_0_2px_4px_rgba(0,0,0,0.1),0_2px_4px_rgba(0,0,0,0.2)]">
          {children}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(255,255,255,0.3),transparent_50%)] opacity-50" />
        </div>
        <div className="absolute w-full h-full bg-gradient-to-br from-[#d0d0c0] to-[#b0b0a0] top-2 left-0 rounded-lg border-2 border-[#2a2a2a] shadow-[0_4px_8px_rgba(0,0,0,0.3)] z-[-1]">
